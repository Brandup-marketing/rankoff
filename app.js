(() => {
  ...
  const elements = {
    ...
    statClicks: document.querySelector("[data-stat-clicks]"),
    statPaid: document.querySelector("[data-stat-paid]"),
    ...
    dialogPrevious: document.querySelector("[data-dialog-previous]"),
    dialogNow: document.querySelector("[data-dialog-now]"),
    dialogAfter: document.querySelector("[data-dialog-after]"),
  ...
  function localizeServerBoard() {
    document.querySelectorAll(".board-seo-note").forEach((node) => {
      ...
      const match = node.dataset.englishCopy.match(/^Sponsored · (.+?) · (.+?) settled · ([\d,]+) tracked clicks$/);
      if (!match) {
        node.textContent = "广告条目";
        return;
      }
      const market = Object.keys(categoryLabels).find((key) => categoryLabels[key] === match[1]);
      node.textContent = `广告 · ${market ? categoryTranslations[market] : match[1]} · ${match[2]} 累计已付 · ${match[3]} 次追踪点击`;
    });
  }
  ...
  function updateBoardSourceLabels() {
    ...
    if (elements.measurementCopy) elements.measurementCopy.textContent = chinese
      ? "追踪点击来自所选时间范围内的第一方跳转记录。重复点击会计入，自动化流量未经过滤，因此一次点击不等于一位独立访客、一条询问或一笔成交。"
      : "Tracked clicks represent first-party redirect events for the selected timeframe. Repeat clicks count and automated traffic is not filtered, so a click is not a unique visitor, enquiry or sale.";
  }
  ...
  function productIdentity(listing, descriptionTag = "p", position = null) {
    ...
    const clickLabel = chinese
      ? `${compact.format(getClicks(listing))} 次${listing.verified ? "追踪" : "推荐"}点击`
      : `${compact.format(getClicks(listing))} ${listing.verified ? "tracked" : "referral"} ${getClicks(listing) === 1 ? "click" : "clicks"}`;
    meta.append(
    ...
    // A bold "0 tracked clicks" on every card reads as a verdict. The age
  ...
  function featuredListingCard(listing, position, ranked, standard = false) {
    ...
    bid.append(
      createElement("span", "", state.activeWindow === "today"
        ...
        : (state.language === "zh" ? "累计已付" : "Total paid")),
  ...
  function renderLeader() {
      ...
      if (boardSource === "production") {
        ...
        if (elements.heroContext) {
          elements.heroContext.textContent = state.language === "zh"
            ? `${money(openingPrice)} 即可登上${state.category === DEFAULT_CATEGORY ? "榜单" : categoryName(state.category, "zh")}第 1 名。成为这里的第一家。`
            : `${money(openingPrice)} takes #1${state.category === DEFAULT_CATEGORY ? "" : ` in ${categoryName(state.category)}`}. Be the first business here.`;
        }
    ...
    if (elements.heroContext) {
      elements.heroContext.textContent = state.language === "zh"
        ? `${money(nextPrice)} 即可登上${state.category === DEFAULT_CATEGORY ? (state.activeWindow === "today" ? "近 24 小时" : "全部时间") : categoryName(state.category, "zh")}榜第 1 名。有人付得更多之前，你的生意一直在这里。`
        : `${money(nextPrice)} takes #1 on the ${state.category === DEFAULT_CATEGORY ? windowLabel().toLowerCase() : categoryName(state.category)} board. Your business stays there until someone pays more.`;
    }
  ...
  // "Who will see my business?" — answered with the board's own counters and
  // nothing else: distinct visitor sessions, redirects to businesses, and the
  // total settled. The strip stays hidden until those numbers arrive.
  let audienceStatsLoaded = false;
  async function loadAudienceStats() {
    if (audienceStatsLoaded || !elements.audienceStrip) return;
    audienceStatsLoaded = true;
    try {
      const response = await fetch("./api/v1/stats?board=global", { headers: { Accept: "application/json" }, cache: "no-store" });
      if (!response.ok) return;
      const stats = await response.json();
      if (stats?.mode !== "production") return;
      const visitors = Number(stats.total_visitors);
      const clicks = Number(stats.total_clicks);
      const paidMinor = Number(stats.settled_revenue_minor);
  ...
  function updateBidPreview() {
    ...
    if (elements.dialogExplanation) {
      ...
      const projection = state.language === "zh"
        ? "你的条目会在付款确认后以此排名上线。其他人仍可付更多取得更高排名。"
