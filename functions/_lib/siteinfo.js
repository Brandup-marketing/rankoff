...
export function extractSiteInfo(html, hostname) {
...
export async function fetchSiteInfo(url, hostname, fetcher = fetch) {
  try {
    ...
    return extractSiteInfo(await response.text(), hostname);
  } catch {
    return { title: "", description: "", logo: "" };
  }
}
