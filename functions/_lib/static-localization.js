...
const STATIC_LOCALIZED_METADATA = Object.freeze({
  ...
  categories: Object.freeze({
    path: "/categories",
    title: "RANKOFF｜行业",
    description: "浏览 Rankoff 行业，查看每个赞助行业中领先的商家。",
    socialTitle: "RANKOFF｜行业",
    socialDescription: "每个行业都有自己的公开赞助榜单。",
  }),
  about: Object.freeze({
    path: "/about",
    title: "关于 RANKOFF",
...
const HOME_STATIC_COPY = Object.freeze({
  ...
  skipLeaderboard: "跳到榜单", navBoard: "榜单", navCategories: "行业", navAbout: "关于",
  heroCopy: "公开赞助榜单，US$1 起。累计付款最高者第 1 名。",
  heroValue: "公开商家页、赞助排名，以及直达你网站的链接。", heroListLabel: "让生意上榜", heroTopLabel: "立即拿下第 1 名", heroExplore: "浏览商家 ↓",
  totalBid: "付款金额", productUrl: "你的网站或社交账号", productUrlPlaceholder: "example.com 或 instagram.com/yourname",
  challengeCategory: "行业", chooseMarket: "选择行业", reviewBid: "确认并付款", markets: "行业",
  todayRanking: "今日领先", boardTotals: "榜单总览", boardRightNow: "此刻的榜单。", seeAll: "查看全部", livePulse: "实时付款", latestActivity: "最新动态", refresh: "刷新",
  ...
  agreeTermsSuffix: "》。", cancel: "取消", continueCheckout: "继续付款", close: "关闭", searchPlaceholder: "搜索商家和行业…",
...
const CATEGORIES_STATIC_COPY = Object.freeze({
  skipCategories: "跳至行业", board: "榜单", categories: "行业", about: "关于", footerParent: "Brandup Marketing 旗下产品",
  rules: "规则", terms: "条款", privacy: "隐私", payments: "付款", seeBoard: "查看榜单", browseMarkets: "浏览行业",
  heroCopy: "每个行业都有自己的榜单。选你的行业，看看谁在榜首。", activeHeading: "最活跃的行业", allHeading: "全部行业", allCopy: "选择一个行业，查看其实时榜单。",
});

const ABOUT_STATIC_COPY = Object.freeze({
  "Skip to content": "跳至正文",
  "What your listing includes →": "上榜后获得哪些展示 →",
  Board: "榜单", Industries: "行业", About: "关于", "The story behind the board": "榜单背后的故事",
  ...
  Now: "现在", "List your business": "让生意上榜", "Enter your website, choose an industry, and pay from US$1.": "输入网站、选择行业，US$1 起付款。",
...
// Apply the board's current price after localisation, before Malay translation.
// Only platform price controls/copy are rewritten: merchant descriptions,
// settled totals, and the live quote for #1 must keep their original values.
// The payment floor and one-unit input step are intentionally different.
export function applyLiveCurrency(html, floor, currency, minimumMinor = null) {
  const value = String(floor || "");
  if (!value) return html;
  const symbol = currency === "MYR" ? "RM" : "US$";
  const currencyName = currency === "MYR" ? "Malaysian ringgit" : "US dollars";
  const priceToken = /(?:US\$|RM)[\s\u00a0]*\d+(?:[,.]\d+)*(?!\d)/g;
  const priceCopy = (copy) => copy.replace(priceToken, () => escapeHtml(value));
  const floorNumber = Number.isSafeInteger(minimumMinor) && minimumMinor > 0
    ? minimumMinor / 100
    : Number(value.replace(/[^\d.]/g, ""));
  let output = String(html)
    .replace(/<title>[\s\S]*?<\/title>|<meta\b[^>]*(?:name="(?:description|twitter:title|twitter:description)"|property="og:(?:title|description)")[^>]*>/gi, priceCopy)
    .replace(/(<([a-z][a-z0-9-]*)\b[^>]*\bdata-i18n="(?:heroCopy|stepPayCopy)"[^>]*>)([\s\S]*?)(<\/\2>)/gi,
    ...
    .replace(/(<span>)(Enter your website,[^<]*pay from (?:US\$|RM)[^<]*|输入网站[^<]*起付款。)(<\/span>)/g,
      (match, open, copy, close) => `${open}${priceCopy(copy)}${close}`)
    .replace(/(<[^>]+\bdata-currency-symbol[^>]*>)[^<]*(<\/[^>]+>)/g,
      (match, open, close) => `${open}${symbol}${close}`)
    .replace(/(<span class="rules-glyph" aria-hidden="true">)(?:US\$|RM)(<\/span>)/g,
      (match, open, close) => `${open}${symbol}${close}`)
    .replace(/(<[^>]+\bdata-dialog-(?:price|previous|now|after)[^>]*>)(?:US\$|RM|\$)[\s\u00a0]*0(?:\.00)?(<\/[^>]+>)/g,
      (match, open, close) => `${open}${symbol} 0${close}`);
  output = output.replace(/<(?:input|button)\b[^>]*>/gi, (tag) => {
    if (/\bdata-inline-bid\b/.test(tag)) {
      const chinese = /aria-label="付款金额/.test(tag);
      const label = chinese
        ? `付款金额（${currency === "MYR" ? "马来西亚令吉" : "美元"}）`
        : `Your payment in ${currencyName}`;
      tag = tag.replace(/\saria-label="[^"]*"/, () => ` aria-label="${label}"`);
      if (Number.isFinite(floorNumber) && floorNumber > 0) {
        tag = tag.replace(/\bmin="[^"]*"/, () => `min="${floorNumber}"`);
      }
      return tag.replace(/\bstep="[^"]*"/, 'step="1"');
    }
    if (/\bdata-inline-adjust=/.test(tag)) {
      return tag.replace(/\saria-label="([^"]*)"/, (match, label) =>
        ` aria-label="${label.replace(priceToken, () => `${symbol}1`)}"`);
    }
    return tag;
  });
  return output;
...
function localizeCommon(html) {
  ...
  output = output.replace(/aria-label="Search businesses and industries"/g, 'aria-label="搜索商家和行业"');
...
export function localizeStaticPage(shell, page, language = "en") {
  ...
  if (page === "home") {
    ...
    // The entry-price node keeps the price token; applyLiveCurrency fills it in.
    html = html.replace(/(<strong[^>]*\bdata-hero-entry-price\b[^>]*>)[\s\S]*?(<\/strong>)/, (match, open, close) => `${open}US$1 起${close}`);
    ...
    html = html.replace(/aria-label="Scroll industries left"/, 'aria-label="向左滚动行业"').replace(/aria-label="Scroll industries right"/, 'aria-label="向右滚动行业"');
    ...
    html = html.replace(/(<p[^>]*data-share-intro>)Send the exact position, board, and industry\.(<\/p>)/, "$1发送准确的排名、榜单和行业。$2");
    ...
    html = html.replace(/(<label[^>]*for="site-search-input"[^>]*>)[\s\S]*?(<\/label>)/, "$1搜索商家和行业$2");
    html = html.replace(/(<input[^>]*data-site-search-input[^>]*placeholder=")[^"]*(")/, "$1搜索商家和行业…$2");
    ...
    html = html.replace(/aria-label="Industry timeframe"/, 'aria-label="行业时间范围"');
